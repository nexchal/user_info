...
function SearchPrint(_user, _res)
{
  g_data = list_userinfolist.UserinfoCreatelist(_user.rows);
  var page = fs.readFileSync(__dirname+'/../views/usersearch.ejs','utf8');
  var dataname = fs.readFileSync(__dirname+'/../views/list_column.ejs','utf8');
  page = ejs.render(page, {
    dbname:dataname ,
    data:g_data,
    button:`<input type="button" onclick="CheckDelete()" value="삭제">`
  });
  _res.writeHead(200);
  _res.end(page);
}
